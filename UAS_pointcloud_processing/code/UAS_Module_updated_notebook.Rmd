...
cropped_dense_point_cloud_fname <- file.path(paste0(wd, "/", file_list[1])) # the number here to say data from which file in the list we want to load
...
test_dsm = raster(file.path(paste0(wd,"/","cropped_dsm_new.tif")))
...
classified_dense_point_cloud <- lidR::classify_ground(las = dense_point_cloud,
                                                      algorithm = csf(sloop_smooth = TRUE, # When sharp slopes exist, smooth to reduced errors.
                                                                      class_threshold = 0.5, #Distance to the simulated cloth to classify point into ground/non-ground.
                                                                      cloth_resolution = 0.25, # distance between points in the point cloud
                                                                      rigidness = 1, #1:fit rugged terrain, 2:medium cloth, 3: hard cloth (for flat terrain) 
                                                                      iterations = 500,
                                                                      time_step = 0.65)) #Time step when simulating the cloth under the gravity. 
...
ttops <- ForestTools::vwf(CHM = raster::raster(chm), 
                          winFun = lin, 
                          minHeight = 2,# change min height based on research objectives 
                          maxWinDiameter = 200) %>% 
   sf::st_as_sf()
...
crowns <-
  ttops %>%
  ForestTools::mcws(CHM = raster::raster(chm), minHeight = 0.5, format = "raster") %>% # # minheight here should be less than minheight in ttops
  setNames(nm = "treeID") %>%
  st_as_stars() %>%
  st_as_sf(merge = TRUE) %>%
