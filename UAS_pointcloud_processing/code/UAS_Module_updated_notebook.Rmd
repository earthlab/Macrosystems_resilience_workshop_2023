...
wd = "/home/jovyan/data_forest_resiliency/UAS/input"
setwd(wd)


# explore the raw data and load the raw data file names

file_list = dir("/home/jovyan/data_forest_resiliency/UAS/input", pattern = ".las", full.names = FALSE, ignore.case = TRUE) 
names_to_cloud = substr(file_list, 2, nchar(file_list)-4) # truncate the file names only to get the name part eliminating the extension.
```
...
plot(dense_point_cloud,color = "Z") # color the points based on elevation or the Z coordinate
...
test_dsm = raster(file.path(paste0("cropped_dsm.tif")))
...
classified_dense_pc <- file.path("data", site_name,flight_datetime, paste0( names_to_cloud[1],"_classified2.las"))
...
cropped_ttops_fname <- file.path("out_data", site_name, flight_datetime, paste0(names_to_cloud[1], "_ttops_cropped2.gpkg"))
...
basemap= raster(file.path(paste0("orthomosaic_NIWOT_plt2.tif"")))
